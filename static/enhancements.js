...
send=async function(text,mode='best'){
  ...
  try{
    ...
    if(d.events?.length){const activity=el('details',undefined,'activity-log');activity.append(el('summary',(lang==='kk'?'Тексеру қадамдары':'Ход проверки')));const names={'Order intent interpreted':['Запрос на заказ разобран','Тапсырыс сұрауы талданды'],'Live stock checked':['Остатки проверены','Қор тексерілді'],'Awaiting explicit confirmation':['Ожидается ваше подтверждение','Растауыңыз күтілуде'],'Requirements extracted':['Требования выделены','Талаптар анықталды'],'Catalog searched':['Каталог проверен','Каталог тексерілді'],'Solution generated':['Подборка готова','Таңдау дайын'],'Live stock rechecked':['Остаток перепроверен','Қор қайта тексерілді'],'Local cart updated':['Корзина обновлена','Себет жаңартылды']};for(const event of d.events){const text=names[event]?.[lang==='kk'?1:0]||(lang==='kk'?'Каталог деректері тексерілді':event);activity.append(el('div','✓ '+text));}$('messages').append(activity);}
